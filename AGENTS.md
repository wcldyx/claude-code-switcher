...
Codex Switcher 是一个 CLI 工具，用于快速切换 Codex 的 API 供应商配置。它允许用户在多个 Codex API 提供商之间切换，管理环境变量，并检测配置冲突。
...
```javascript
{
  ...
  providers: {
    'provider-name': {
      ...
      models: {
        primary: 'Codex-sonnet-4',
        smallFast: 'Codex-haiku-4'
      },
      launchArgs: ['--continue'],
      current: true,
...
`src/utils/Codex-settings.js` 实现了自动检测和处理 Codex 设置文件中的环境变量冲突：

**工作原理**：
1. 扫描多个可能的设置文件位置（`.Codex/settings.json`, `.Codex/settings.local.json`）
...
`src/utils/env-launcher.js` 负责根据供应商配置设置环境变量并启动 Codex：
...
│   └── current.js     # 显示当前供应商
├── utils/             # 工具模块
│   ├── Codex-settings.js  # Codex 设置文件冲突检测
